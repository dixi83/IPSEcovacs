...
class EcoVacsAPI:
    ...
    def __init__(self, device_id, account_id, password_hash, country, continent):
        ...
        _LOGGER.error("device_id:      {}".format(device_id))
        _LOGGER.error("device_id[0:8]: {}".format(device_id[0:8]))
    ...
    def __sign(self, params):
        ...
        _LOGGER.error("sign_on_text: {}".format(sign_on_text))
    ...
    def __call_main_api(self, function, *args):
        ...
        _LOGGER.error("function txt: {}".format(function))
        _LOGGER.error("    sign txt: {}".format(self.__sign(params)))
        api_response = requests.get(url, self.__sign(params))
        _LOGGER.error("     URL txt: {}".format(api_response.url))
        json = api_response.json()
        _LOGGER.error("got {}".format(json))
        if json['code'] == '0000':
            return json['data']
        elif json['code'] == '1005':
...
class VacBot():
    ...
    def _vacuum_address(self):
        _LOGGER.debug("username to send to vac: " + self.vacuum['did'] + "@" + self.vacuum['class'] + ".ecorobot.net/atom")
...
class EcoVacsXMPP(ClientXMPP):
    def __init__(self, user, domain, resource, secret, continent, server_address=None):
        _LOGGER.debug("username used to login: " + user + "@" + domain)
        _LOGGER.debug("password used to login: 0/" + resource + "/" + secret)
